...
try:
    ...
    (load_file_path, temfilename2) = os.path.split(Output_pathfile)
    (load_file_name, load_file_end) = os.path.splitext(temfilename2)
    Output_path = load_file_path

    (STLfilepath, tempfilename3) = os.path.split(Output_STLpathfile)
    (STLfilename, extension3) = os.path.splitext(tempfilename3)

    Load_File_File = Output_path + SyetemSlash + load_file_name + "_VTK"
    MakeFile(Load_File_File)

    Load_Con_File = Output_path
...
Face_stl_path = STLfilepath + SyetemSlash + STLfilename + ".stl"
...
# 创建一个文件，作为vtm
Face_VTM = Load_Con_File + SyetemSlash + load_file_name + ".vtm"
...
#打开之间的vtm文件，并续写内容
wireVTM_file = Load_Con_File + SyetemSlash + load_file_name +".vtm"
