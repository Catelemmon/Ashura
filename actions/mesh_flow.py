...
class MeshControler(object):
    ...
    def start_actions(self):
        core_logger.info(f"划分网格信息写入数据库 | mesh_APP: {self.mesh_app} | work_path: {self.work_path} | cad:name: {self.cad_file_name}"
        ...
        slurm = Slurm()
        slurm_id = slurm.send_job(self.work_path, launch_script=self.lanuch_script, username=self.username, total_core=10,
